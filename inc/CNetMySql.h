...
typedef struct sMySqlConnection MySqlConnection;

/**
 * Open a connection to MySQL/MariaDB via ODBC.
 *
 * @param gc               Allocator that owns the returned connection.
 * @param connectionString Wide ODBC connection string, e.g.
 *                          L"Driver={MySQL ODBC 8.0 Driver};Server=localhost;".
 * @return A newly allocated MySqlConnection, owned by gc, or NULL if the
 *         database feature is not licensed or the ODBC connect call fails.
 */
CNET_API MySqlConnection* mySqlOpen(Gc* gc, const wchar_t* connectionString);
 ...
 */
CNET_API void mySqlClose(MySqlConnection* conn);
 ...
 *         or NULL on error.
 */
CNET_API DataSet* mySqlQuery(Gc* gc, MySqlConnection* conn, const wchar_t* sql, const DataValue* params, int paramCount);
 ...
 *         detect errors.
 */
CNET_API int mySqlExecute(MySqlConnection* conn, const wchar_t* sql, const DataValue* params, int paramCount);
